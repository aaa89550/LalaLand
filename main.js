...
function appendMessage(msg, msgId) {
  ...
  if (msg.type === "poll") {
  const votedIndex = msg.votes?.[currentUser.uid];
  const totalVotes = Object.keys(msg.votes || {}).length;

  let pollHtml = `<strong>${msg.question}</strong><br>`;
  msg.options.forEach((opt, i) => {
    const count = Object.values(msg.votes || {}).filter(v => v === i).length;
    const percent = totalVotes ? Math.round((count / totalVotes) * 100) : 0;

    if (votedIndex === i) {
      pollHtml += `<div style="margin: 6px 0; color: green;">✔️ ${opt} (${count}票, ${percent}%)</div>`;
    } else if (votedIndex !== undefined) {
      pollHtml += `<div style="margin: 6px 0;">${opt} (${count}票, ${percent}%)</div>`;
    } else {
      pollHtml += `<button onclick="vote('${msgId}', ${i})">${opt}</button><br>`;
    }
  });

  div.querySelector('.bubble').innerHTML = pollHtml;
 }
}
